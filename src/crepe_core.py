...
def build_and_load_model(model_capacity):
    ...
    if models[model_capacity] is None:
        ...
        mod = Model(inputs=x, outputs=y)

        package_dir = os.path.dirname(os.path.realpath(__file__))
        weights_dir = os.path.join(package_dir, 'crepe_weights')
        filename = "model-{}.h5".format(model_capacity)
        mod.load_weights(os.path.join(weights_dir, filename))
        mod.compile('adam', 'binary_crossentropy')

        models[model_capacity] = mod
